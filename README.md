...
- **AI Text Generation** — Gemini 1.5 Flash generates Twitter posts, captions, and hashtags
...
>
> ⚠️ First request may take 20–40s if the model is "cold" (loading). Subsequent requests are faster.

**Recommended image models** (set in `HUGGINGFACE_IMAGE_MODEL`):

| Model | Speed | Quality |
|-------|-------|---------|
| `stabilityai/stable-diffusion-xl-base-1.0` | Slow | ⭐⭐⭐⭐⭐ |
| `runwayml/stable-diffusion-v1-5` | Medium | ⭐⭐⭐⭐ |
| `stabilityai/sdxl-turbo` | Fast | ⭐⭐⭐ |
...
> ⚠️ Twitter's free tier (Basic) allows **1,500 tweets/month**. The agent respects this.
